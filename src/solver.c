...
double evaluate (short *Q, int maxNodes,double **val,double *Qval,double *Row,double *Col)
{
  ...
  for (i = 0; i < maxNodes; i++) {
      ...
      for ( j = i+1; j < maxNodes; j++) { Row[i]+= val[i][j]*(double)Q[j]; }
      for ( j = 0  ; j < i       ; j++) { Col[i]+= val[j][i]*(double)Q[j]; }
...
if( Tlist_ != -1 ) {
    ...
    if ( maxNodes <= 50    && maxNodes < 1 )     { nTabu = 10;goto brk;}
    if ( maxNodes <= 100   && maxNodes <= 249  ) { nTabu = 12;goto brk;}
    if ( maxNodes <= 250   && maxNodes <= 499  ) { nTabu = 13;goto brk;}
    if ( maxNodes <= 500   && maxNodes <= 999  ) { nTabu = 21;goto brk;}
    if ( maxNodes <= 1000  && maxNodes <= 2499 ) { nTabu = 29;goto brk;}
    if ( maxNodes >= 2500  && maxNodes <= 7999 ) { nTabu = 34;goto brk;}
    if ( maxNodes >= 8000 ) { nTabu = 35;}
}
brk:
...
//
if (GETMEM(Q        ,short  ,maxNodes) == NULL) { BADMALLOC }
if (GETMEM(Qt       ,short  ,maxNodes) == NULL) { BADMALLOC }
if (GETMEM(Qval     ,double ,maxNodes) == NULL) { BADMALLOC }
if (GETMEM(Row      ,double ,maxNodes) == NULL) { BADMALLOC }
if (GETMEM(Col      ,double ,maxNodes) == NULL) { BADMALLOC }
if (GETMEM(index    ,int    ,maxNodes) == NULL) { BADMALLOC }
if (GETMEM(TabuK    ,int    ,maxNodes) == NULL) { BADMALLOC }
//
...
for (i=0;i<QLEN+1;i++){
    ...
    for ( j=0;j<maxNodes;j++ ) { Qlist[i][j]=0; }
}
//
...
if (GETMEM(Icompress  ,int   ,maxNodes)   == NULL) {BADMALLOC}
if (GETMEM(Qbest      ,short ,maxNodes)   == NULL) {BADMALLOC}
...
if (Verbose_>1) {DLT;printf(" V Starting outer loop =%lf iterations %lld\n",Vbest*fmin,t); }
//
...
// begin submatrix passes
   ...
   if( Verbose_>1 )  {DLT;printf(" ***Full Tabu  -- after partition pass \n"); }
   //    FULL TABU run here
   ...
   if ( NU == 10 ) {   // better solution
         ...
         RepeatPass++;
         NoProgress++;
         if( Verbose_>1) {printf("NO improvement RepeatPass =%d\n",RepeatPass);}
   }

   if ( Verbose_>1) {DLT;printf("V Best outer loop =%lf iterations %lld,  %s\n",Vbest*fmin,t,inFileNm_);}
   ...
   if ( (double) (clock()-start_)/CLOCKS_PER_SEC >= Time_ ) {ContinueWhile=FALSE;} // timeout test
} // end of outer loop
