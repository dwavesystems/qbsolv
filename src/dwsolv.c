...
int my_pid_;
char DWcommand[256];
char filename_b[256],filename_result[256];
...
dw_init ()
{
    ...
    // .epqmi file and path needed to embed problems to the solver
      ...
      if ( wspath == NULL ) {
          ...
          if ((fs = fopen(filename_max_full,"r")) == NULL) { printf(" no file %s\n",filename_max_full); exit(9); }
          if ( fscanf(fs,"%s",linebuf) == 0 ) { DL;printf("fscanf error");exit(9);}
          fclose(fs);
      ...
      if ((fs = fopen(filename_epqmi_max,"r")) == NULL) { printf(" no file %s\n",filename_epqmi_max); exit(9); }
        if ( fscanf(fs,"%d",&S_embed) == 0 ) { DL;printf("fscanf error");exit(9);}
        SubMatrix_=S_embed;
      fclose(fs);
...
void
dw_solver (double **val, int maxNodes, short *Q )
{
...
// 
      ...
      if ((f = fopen(filename_b,"w")) == NULL ) { DL;printf("  Failed open %s\n",filename_b); exit(9); }

      // dwave finds minimum, while our tabu finds max, so send negative to dwave
      for (i=0;i<maxNodes;i++) { fprintf(f,"a_%d = %6.3f\n",i+1,-val[i][i]); }
      for (i=0;i<maxNodes;i++) {
         for (j=i+1;j<maxNodes;j++) {fprintf(f,"b_%d_%d = %6.3f\n",i+1,j+1,-val[i][j]); } }
      fclose(f);
      ...
      if ((fr = fopen(filename_result,"r")) == NULL ) { DL;printf("  Failed open %s\n",filename_result); exit(9); }
      short qtmp;
      for (i=0;i<maxNodes;i++) { 
            if ((fscanf(fr,"%hd",&qtmp)) == 0 )  { DL;printf("fscanf error %s",filename_result);exit(9);}
            Q[i]=qtmp;
      }
      if (Verbose_>3){
...
void dw_close() {
    ...
    if (system(DWcommand) != 0 ) { DL;printf("system call error,%s",DWcommand);} // clean up the /tmp directory

    sprintf(DWcommand,"rm -f %s/%s/qbs%d.* \n",workspace,ws_tmp_path,my_pid_);

    if (system(DWcommand) != 0 ) { DL;printf("system call error,%s",DWcommand);} // clean up the dw full.x directory
