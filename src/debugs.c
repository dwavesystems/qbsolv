...
double just_evaluate (short *Q, int maxNodes,double **val)
{
  int i, j;
  double result=0.0;
  for (i = 0; i < maxNodes; i++) { for ( j = i; j < maxNodes; j++) { result+= val[i][j]*(double)Q[j]*(double)Q[i]; } }
  return result;
}
...
void check_row_col_qval (short *Q, int maxNodes,double **val,double *Qval,double *Row,double *Col)
{
  ...
  for (i = 0; i < maxNodes; i++) {
      ...
      for ( j = i+1; j < maxNodes; j++) { Rows+= val[i][j]*(double)Q[j]; }
      for ( j = 0  ; j < i       ; j++) { Cols+= val[j][i]*(double)Q[j]; }
